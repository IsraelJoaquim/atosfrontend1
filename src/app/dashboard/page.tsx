...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total', value: tickets.length, icon: Ticket, color: 'text-accent-cyan', bg: 'bg-accent-cyan_dim' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {stats.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} className="card p-4 md:p-5">
            ...
            <p className={`text-2xl md:text-3xl font-display font-bold mt-1 ${color}`}>
              {loading ? '—' : value}
            </p>
          </div>
        ))}
      ...
      <div className="card overflow-hidden">
        <div className="p-4 md:p-5 border-b border-bg-border flex items-center justify-between">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-bg-border">
            {tickets.slice(0, 8).map((t) => (
              <div key={t.id} className="px-4 md:px-5 py-3.5 flex items-center gap-3 md:gap-4 hover:bg-bg-border/30 transition-colors">
                <span className="text-text-muted font-mono text-xs w-20 md:w-24 shrink-0">{t.ticket}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-text-primary text-sm truncate">{t.title}</p>
                  {showOpener && t.user && (
                    <p className="text-text-muted text-xs font-mono truncate">{t.user.email}</p>
                  )}
                </div>
